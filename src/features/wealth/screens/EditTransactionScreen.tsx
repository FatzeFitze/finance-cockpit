...
import { getTransactionById, listAccountsByPortfolio, listAssets, listPortfolios, updateTransaction } from '../data/wealth.repository';
...
export default function EditTransactionScreen() { const db = useSQLiteContext(); const router = useRouter(); const params = useLocalSearchParams<{ id?: string }>(); const [item, setItem] = useState<WealthTransaction | null>(null); const [accounts, setAccounts] = useState<Account[]>([]); const [assets, setAssets] = useState<Asset[]>([]); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false);
  useEffect(() => { let active = true; void (async () => { if (!params.id) return; const [transaction, portfolios, nextAssets] = await Promise.all([getTransactionById(db, params.id as TransactionId), listPortfolios(db), listAssets(db)]); const accountsByPortfolio = await Promise.all(portfolios.map((portfolio) => listAccountsByPortfolio(db, portfolio.id))); if (active) { setItem(transaction); setAccounts(accountsByPortfolio.flat()); setAssets(nextAssets); setLoading(false); } })(); return () => { active = false; }; }, [db, params.id]);
